...
async function start(page: Page, state = progressed()) {
  await page.addInitScript(state => { if (!localStorage.getItem('elderwood-save-v4')) localStorage.setItem('elderwood-save-v4', JSON.stringify(state)); }, state);
...
test('corrupt saves stay intact until recovery is chosen; backup and old save remain', async ({ page }) => {
  ...
  await page.addInitScript(raw => { localStorage.setItem('elderwood-save-v1', raw); localStorage.setItem('elderwood-save-v4', 'damaged-data'); }, raw);
  ...
  expect(await page.evaluate(() => localStorage.getItem('elderwood-save-v4'))).toBe('damaged-data');
  await page.locator('[data-recover="legacy"]').click(); await expect(page.locator('#modal-backdrop')).toBeHidden();
  const storage = await page.evaluate(() => ({ old:localStorage.getItem('elderwood-save-v1'), state:JSON.parse(localStorage.getItem('elderwood-save-v4')!), backups:Object.keys(localStorage).filter(k => k.startsWith('elderwood-save-v4-backup-')).map(k => localStorage.getItem(k)) }));
