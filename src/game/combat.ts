...
export class Combat {
  ...
  activateSeal(index: number): boolean {
    ...
    if (!this.alive || !boss || boss.bossPhase !== 3 || !SEALS[index] || distance(this.player, SEALS[index]) > 3.2) return false;
  ...
  private updateEnemy(e: Enemy, dt: number) {
    ...
    if (e.mode === 'attack' && e.telegraph?.kind === 'charge') {
      ...
      if (!e.chargeHit && segmentDistance(this.player, old, e.pos) <= e.telegraph.width + 0.4 && clearLine(old, this.player, this.obstacles)) { e.chargeHit = true; this.hurt(data.attack, old, true, e); }
      e.timer -= dt;
  ...
  private executeEnemyAttack(e: Enemy) {
    ...
    const data = MONSTERS[e.species];
    if (t.kind === 'shot') this.shoot(t.origin, t.yaw, data.attack, t.range, t.width, false, 'none', data.attack, e.id);
    else {
      ...
      if (hit && clearLine(t.origin, this.player, this.obstacles)) this.hurt(data.attack * (t.kind === 'fire' ? 1.2 : 1), t.origin, t.kind !== 'fire' && t.shape !== 'circle', e);
