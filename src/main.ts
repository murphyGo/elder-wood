...
import { materialInventory, workshopHTML, journeyJournalHTML, expansionObjectivesHTML, worldMapHTML } from './journey-ui';
import { isSafeZone, isBoss } from './game/state';
...
function updateNameplates() {
  ...
  document.querySelectorAll<HTMLElement>('.world-nameplate').forEach((label, index) => {
    ...
    label.style.display = game.siteActive(landmark) && !covered && p.z < 1 && p.z > -1 && Math.abs(p.x) < 0.85 && Math.abs(p.y) < 0.85 && distance < 28 ? '' : 'none';
...
function drawMinimap() {
  ...
  if (isSafeZone(game.state.zone)) for (const [x, y] of [[-63, -26], [-102, -50], [-35, -80]]) { c.save(); c.translate(x, y); c.rotate(0.2); c.fillStyle = '#c4b286'; c.fillRect(-10, -8, 20, 16); c.strokeStyle = '#857348'; c.strokeRect(-10, -8, 20, 16); c.restore(); }
  }
  for (const l of game.environment.landmarks) { if (!game.siteActive(l)) continue; const x = l.x * 4.7; const z = l.z * 4.7; c.fillStyle = l.kind === 'portal' ? '#acd4c0' : '#ebcf85'; c.save(); c.translate(x, z); c.rotate(Math.PI / 4); c.fillRect(-3.5, -3.5, 7, 7); c.restore(); }
  for (const e of game.enemies) if (e.hp > 0) { c.fillStyle = isBoss(e.species) ? '#e8a2bb' : '#dc947b'; c.beginPath(); c.arc(e.model.position.x * 4.7, e.model.position.z * 4.7, isBoss(e.species) ? 6 : 3, 0, 6.28); c.fill(); }
...
function openPanel(panel: string) {
  ...
  if (panel === 'character') shell('YOUR ADVENTURER', '나의 캐릭터', `<div class="character-layout"><div class="character-portrait">${portrait}<span>Lv. ${s.level}</span></div><div><p class="character-class">${s.journey.step === 7 ? '숲과 바다의 수호자' : s.chapter === 6 ? '숲의 수호자' : '엘더우드의 여행자'}</p><h3>여행자</h3><p class="muted">작은 발걸음이, 숲의 운명을 바꿉니다.</p><div class="stats-grid"><div>${icon('heart')}<span>최대 체력</span><b>${maxHp(s)}</b></div><div>${icon('drop')}<span>최대 마력</span><b>${maxMp(s)}</b></div><div>${icon('sword')}<span>공격력</span><b>${attackPower(s)}</b></div><div>${icon('shield')}<span>방어력</span><b>${defense(s)}</b></div></div></div></div><div class="modal-section-title">배운 스킬 <span>레벨을 달성하면 자동으로 습득합니다</span></div><div class="skill-list">${SKILL_KEYS.map(k => [k, getSkill(s, k)] as const).map(([k, skill]) => `<div class="skill-detail ${s.level < skill.level ? 'unlearned' : ''}"><div class="detail-icon">${icon(skill.icon)}</div><div><b>${skill.name}</b><p>${skill.description}</p><small>MP ${skill.mana} · 재사용 ${skill.cooldown}초</small></div><span>${s.level < skill.level ? `Lv. ${skill.level} 해금` : `<kbd>${k.toUpperCase()}</kbd> 습득 완료`}</span></div>`).join('')}</div><p class="modal-footnote">다음 레벨까지 ${xpRequired(s.level) - s.xp} EXP · 총 ${s.totalKills}마리 해방 · 모험 시간 ${Math.floor(s.playTime / 60)}분</p>`, true);
    ...
    if (!isSafeZone(s.zone)) { closePanel(); game.toast('그린헤이븐 마을에서 상인을 만나세요.'); return; }
...
function claimQuest() {
  if (game.state.chapter === 6) {
    ...
    game.killNotice = false; game.save(); game.refreshStory(); render(); game.sound.play('level');
    activePanel = 'reward'; game.setPaused(true); $('#modal-backdrop').classList.remove('hidden');
    shell('THE TIDE TURNS', step === 6 ? '검은 조수 · 이야기 완료' : '또 하나의 불빛', `<div class="reward-symbol">${icon('sun')}</div><p class="dialogue-quote">${q.ending}</p><div class="reward-prizes"><div><b>+${q.xp}</b><span>경험치</span></div><div><b>+${q.gold}</b><span>골드</span></div><div><b>+2</b><span>회복 물약</span></div></div>${step === 1 ? '<p class="help-tip">등불 제작 재료: 표류목 3 · 철 3 · 별의 가루 2</p>' : ''}<button class="primary-button centered" data-next-chapter>${step === 6 ? '항구로 돌아가기' : '다음 여정으로'} ${icon('arrow')}</button>`);
    modal.scrollTop = 0; modal.focus(); if (game.state.level > level) handleEvent({ type: 'level', text: `레벨 ${game.state.level}` }); return;
  }
...
document.addEventListener('click', event => {
  ...
  if (target.hasAttribute('data-shop-visit')) {
    closePanel(); if (!isSafeZone(game.state.zone)) game.travel(game.environment.coast ? 'harbor' : 'village');
    if (game.state.zone === 'harbor') game.hero.position.set(-3, 0, 16.5); else game.hero.position.set(-10, 0, 2.7);
    openPanel('shop');
  }
